...
public class Wood {
	...
	public static class Shapeless {
		...
		public static HashMap<ShapelessRecipeBuilder, ResourceLocation> getRecipes() {
			...
			for (Item log : ItemLists.LOGS) {
				Item strippedLog = ForgeRegistries.ITEMS.getValue(new ResourceLocation(log.getRegistryName().getNamespace(),
						"stripped_" + log.getRegistryName().getPath()));
				
				Item wood = ForgeRegistries.ITEMS.getValue(new ResourceLocation(log.getRegistryName().getNamespace(),
						log.getRegistryName().getPath().replace("log", "wood")));
				Item strippedWood = ForgeRegistries.ITEMS.getValue(new ResourceLocation(wood.getRegistryName().getNamespace(),
						"stripped_" + wood.getRegistryName().getPath()));
