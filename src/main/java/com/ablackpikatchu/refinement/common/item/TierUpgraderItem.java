...
import com.ablackpikatchu.refinement.api.event.TileTierChangeEvent;
	...
	public ActionResultType useOn(ItemUseContext pContext) {
		if (!pContext.getLevel().isClientSide()) {
			if (pContext.getLevel().getBlockEntity(pContext.getClickedPos()) instanceof ITieredTile) {
				...
				if (tieredTile.getCurrentTier().nextTier() != null
						&& tieredTile.getCurrentTier().nextTier() == this.tier
						&& !TileTierChangeEvent.onTileTierChange(pContext.getLevel(), pContext.getClickedPos())) {
