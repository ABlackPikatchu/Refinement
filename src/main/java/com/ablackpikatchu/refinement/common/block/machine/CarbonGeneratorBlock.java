...
public class CarbonGeneratorBlock extends Block {
	...
	public TileEntity createTileEntity(BlockState state, IBlockReader world) {
		return TileEntityTypesInit.CARBON_GENERATOR_TILE_ENTITY_TYPE.get().create();
	}
	...
	public ActionResultType use(BlockState state, World worldIn, BlockPos pos, PlayerEntity player,
		...
		if (!worldIn.isClientSide()) {
			...
			if (te instanceof CarbonGeneratorTileEntity) {
				NetworkHooks.openGui((ServerPlayerEntity) player, (CarbonGeneratorTileEntity) te, pos);
			}
		}
	...
	public void setPlacedBy(World worldIn, BlockPos pos, BlockState state, LivingEntity placer,
		...
		if (stack.hasCustomHoverName()) {
			...
			if (tile instanceof CarbonGeneratorTileEntity) {
				((CarbonGeneratorTileEntity) tile).setCustomName(stack.getDisplayName());
			}
		}
	}
